...
state,mergedAt`, or `git ls-remote --heads origin <branch>`). This repo auto-deletes a branch once
its PR merges. Pushing to that now-deleted name doesn't reopen or add to the merged PR — it
silently creates a fresh, disconnected branch with the same name, and whatever you push there has
no PR at all until you notice and open one. Always start a new branch off latest `main` unless
you've just confirmed the target branch/PR is still open.
