...
export namespace opentelemetry.proto.common.v1 {
    ...
    export class InstrumentationScope extends pb_1.Message {
        ...
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): InstrumentationScope {
            const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new InstrumentationScope();
            while (reader.nextField()) {
                if (reader.isEndGroup())
                    break;
                switch (reader.getFieldNumber()) {
